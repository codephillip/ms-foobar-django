venv_path := ~/.venv/Foobar_auth_app
...
shell:
	@echo 'To activate the venv use source ~/.venv/Foobar_auth_app/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 Foobar_auth_app -r -d
	${bin_path}/autopep8 Foobar_auth_app -r -i
	@echo '---Running isort---'
	${bin_path}/isort Foobar_auth_app --diff
	${bin_path}/isort Foobar_auth_app --atomic
